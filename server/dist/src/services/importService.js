...
exports.analyzeCSVData = void 0;
const sync_1 = require("csv-parse/sync");
const prisma_1 = __importDefault(require("../utils/prisma"));
const EXCHANGE_RATE_USD_TO_INR = 83;
const analyzeCSVData = async (buffer, groupId) => {
    ...
    for (let i = 0; i < records.length; i++) {
        ...
        const splitWithNames = row.split_with ? row.split_with.split(';').map((s) => s.trim().toLowerCase()) : [];
        let splitType = (row.split_type || 'equal').toUpperCase();
        // Validate split type
        if (!['EQUAL', 'EXACT', 'PERCENTAGE', 'SHARE'].includes(splitType)) {
            splitType = 'EQUAL';
        }
        const splitDetails = row.split_details ? row.split_details.split(';').map((s) => s.trim()) : [];
...
exports.analyzeCSVData = analyzeCSVData;
